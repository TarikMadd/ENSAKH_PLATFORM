 ...
 */
?>
<form method="post" action="AddPaimentsup3">
                ...
                <div class="box-body">
                    <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                        ...
                        $months = array (1=>'Janvier',2=>'février',3=>'Mars',4=>'Avril',5=>'Mai',6=>'Juin',7=>'juillet',8=>'aout',9=>'septembre',10=>'octobre',11=>'novembre',12=>'décembre');
                         foreach ($suppheures as $suppheure): ?>
                            <tr>
                                <td><?= $this->Number->format($suppheure->id) ?></td>
                                <td><?= h($suppheure->annee) ?></td>
                                <td><?= $months[$suppheure->mois] ?></td>

                                <td><?= $this->Number->format($suppheure->nbHeure) ?></td>
                                <?php
                                if($suppheure->etat=="non validé") {
                                    echo "<td><span class=\"label label-danger\">$suppheure->etat</span></td>";
                                }else if($suppheure->etat=="valide")
                                    echo "<td><span class=\"label label-primary\">$suppheure->etat</span></td>";
                                else
                                    echo "<td><span class=\"label label-success\">$suppheure->etat</span></td>";
                                ?>

                                <td><label>
                                        <input type="checkbox" name="supheures[]" class="flat-red" value="<?= h($suppheure->id) ?>">
                                    </label></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
