...
<p> Dans le but de développer davantage l’esprit d’initiative chez l’élève ingénieur de notre établissement, nous vous demandons d’accueillir <?php if($donne[0]['code_sexe']=="m"){echo "M "; }else { echo "Mlle ";} ?> <span style="font-weight: bold;"><?= $donne[0]['nom_fr'] ?> <?=$donne[0]['prenom_fr']?></span> , <?php if($donne[0]['code_sexe']=="m"){echo "inscrit";}else { echo "inscrite ";} ?>  2 ème année du cycle ingénieur, Filière <span style="font-weight: bold;"><?= $donne[0]['libile'] ?> </span> sous le CNE n°  <span style="font-weight: bold;"><?= $donne[0]['cne'] ?></span> ,afin d’effectuer un stage d’ingénieur adjoint  au sein de votre établissement de deux mois, du <?= h($donne[0]['debut_stage']) ?> au <?= h($donne[0]['fin_stage']) ?>  </p>

<p>
Ce stage sera sanctionné par un rapport à déposer à la direction au début de l’année universitaire en cours et suivi d’une soutenance en Septembre 2017.
</p>
